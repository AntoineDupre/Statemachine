...
try:
    from collections import \
        OrderedDict  # in python 2.7 and up
...
class Exit(object):
    """Representation of an exit from one State to another. Optionally
    includes a condition (callable) that must be True for the exit to
    be available, and an action (callable) that will be run before the
    exit is taken."""
...
class State(object):
    """Representation of a state. Has zero or more Exits and an optional
    action which will be run before the State is entered."""
...
class StateMachine(object):
    def __init__(self, states=None, start=None,
        ...
        self.history = [(time.time(), self.start)]
    ...
    @property
    def finished(self):
        "Returns whether we're in an end state or not."
